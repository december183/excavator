...
class AdverTypeController extends BaseController{
        ...
        if(IS_POST){
            ...
            if($data['action'] == 1){
                ...
                if($this->type->delete($ids)){
                    $this->apiReturn(200,'批量删除成功',array('url'=>'index.php?s=Admin/AdverType/index'));
                }else{
                ...
                $this->apiReturn(200,'批量设置成功',array('url'=>'index.php?s=Admin/AdverType/index'));
            }
        }else{
    ...
    public function add(){
        if(IS_POST){
            ...
            if($this->type->create($data)){
                if($this->type->add()){
                    $this->apiReturn(200,'新增广告类型成功',array('url'=>'index.php?s=Admin/AdverType/index'));
                }else{
    ...
    public function edit(){
        if(IS_POST){
            ...
            if(!$this->type->checkName($data['id'],$data['name'])){
                $this->apiReturn(403,'广告类型名称不能重复');
            }
            if($this->type->create($data)){
                if($this->type->save()){
                    $this->apiReturn(200,'修改广告类型成功',array('url'=>'index.php?s=Admin/AdverType/index'));
                }else{
    ...
    public function getTypeInfo(){
        $data['id']=I('param.id');
        $oneType=$this->type->field('width,height')->where($data)->find();
        if($oneType){
            $this->apiReturn(200,'获取广告类型信息成功',$oneType);
        }else{
            $this->apiReturn(404,'获取广告类型信息失败');
        }
    }
