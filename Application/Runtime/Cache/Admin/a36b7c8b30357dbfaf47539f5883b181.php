...
<section class="vbox">
    ...
    <ul class="nav navbar-nav navbar-right hidden-xs nav-user">
        ...
        <li class="dropdown"> <a href="#" class="dropdown-toggle" data-toggle="dropdown"> <span class="thumb-sm avatar pull-left"> <img src="/excavator/Public/Admin/images/avatar.jpg"> </span> John.Smith <b class="caret"></b> </a>
            <ul class="dropdown-menu animated fadeInRight">
                ...
                <li class="divider"></li>
                <li> <a href="/excavator/index.php?s=/Admin/Accessory/logout" data-toggle="ajaxModal" >Logout</a> </li>
            </ul>
        </li>
    </ul>
</header>
        ...
        <section class="vbox">
            <section class="scrollable padder">
                <ul class="breadcrumb no-border no-radius b-b b-light pull-in">
                    ...
                    <li><a href="/excavator/index.php?s=/Admin/Accessory/index">配件列表</a></li>
                    <li><a href="/excavator/index.php?s=/Admin/Accessory/add">新增配件</a></li>
                    <li><a href="/excavator/index.php?s=/Admin/Accessory/edit/id/<?php echo ($oneAcces["id"]); ?>">修改配件</a></li>
                </ul>
                <div class="m-b-md">
                    <h3 class="m-b-none">修改配件</h3>
                </div>
                <div class="row">
                    <div class="col-sm-12">
                        <section class="panel panel-default">
                            <header class="panel-heading font-bold">修改配件</header>
                            <div class="panel-body">
                                <form class="bs-example form-horizontal" id="ajaxForm" action="/excavator/index.php?s=/Admin/Accessory/edit">
                                    <input type="hidden" name="id" value="<?php echo ($oneAcces["id"]); ?>" />
                                    <input type="hidden" id="brand" value="<?php echo ($oneAcces["brand"]); ?>" />
                                    <div class="form-group">
                                        <label class="col-lg-2 control-label">配件型号</label>
                                        <div class="col-lg-4">
                                            <input type="text" class="form-control" name="accesno" placeholder="Accesno" value="<?php echo ($oneAcces["accesno"]); ?>">
                                            <span class="help-block m-b-none">配件型号不能小于2位.</span>
                                        </div>
                                        <div class="col-lg-6"></div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-lg-2 control-label">配件分类</label>
                                        <div class="col-lg-4">
                                            <select name="cateid" id="cateid" class="form-control m-b">
                                                <option value="0">请选择配件分类</option>
                                                <?php if(is_array($catelist)): $i = 0; $__LIST__ = $catelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo['id'] == $oneAcces['cateid']): ?>selected="selected"<?php endif; ?>><?php echo ($vo["name"]); ?></option>
                                                    <?php if(is_array($vo['child'])): $i = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($voo["id"]); ?>" <?php if($voo['id'] == $oneAcces['cateid']): ?>selected="selected"<?php endif; ?>><?php echo ($voo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
                                            </select>
                                        </div>
                                        <div class="col-lg-6"></div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-lg-2 control-label">相关品牌</label>
                                        <div class="col-lg-4">
                                            <select name="brand" id="brandid" class="form-control m-b">
                                                <option value="0">选择相关品牌</option>
                                            </select>
                                        </div>
                                        <div class="col-lg-6"></div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-2 control-label">配件图片</label>
                                        <div class="col-sm-10">
                                            ...
                                            <a href="#" class="thumb-sm"><img src="<?php echo ($oneAcces["thumbpic"]); ?>" class="img-rounded"></a>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-2 control-label">关联机型</label>
                                        <div class="col-sm-10">
                                            <?php if(is_array($machinelist)): $i = 0; $__LIST__ = $machinelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><label class="checkbox pad-top top-cate"><?php echo ($vo["name"]); ?></label>
                                                <?php if(is_array($vo['child'])): $i = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?><label class="checkbox-inline sub-cate">
                                                        <input type="checkbox" name="ids[]" value="<?php echo ($voo["id"]); ?>" <?php if(in_array(($voo['id']), is_array($oneAcces['machids'])?$oneAcces['machids']:explode(',',$oneAcces['machids']))): ?>checked="checked"<?php endif; ?>> <?php echo ($voo["machineno"]); ?> </label><?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="col-lg-offset-2 col-lg-10">
                                            <button type="submit" class="btn btn-sm btn-default">修改配件</button>
                                        </div>
                                    </div>
                                </form>
                            </div>
    ...
    <script src="/excavator/Public/Admin/js/accessory/index.js"></script>
